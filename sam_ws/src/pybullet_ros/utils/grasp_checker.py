...
class ValidGraspChecker():
    ...
    def draw_aabb(self, target_id, lifetime=5, cabinet_stage=2):
        ...
        for joint_index in range(num_joints):
            info = p.getJointInfo(target_id, joint_index)
            print(f"Joint {joint_index}: {info}")


        if cabinet_stage == 2:
            target_box = p.getAABB(target_id, 1)
    ...
    def extract_grasp(self, grasp, drawback_distance=0., visual=False, filter_elbow=True, time=0.2, distance=0.0001):
        ...
        for i in range(grasp.shape[0]):
            ...
            #     continue
            if len(p.getClosestPoints(self.robot, self.env.cabinet_id, distance)):
                continue
